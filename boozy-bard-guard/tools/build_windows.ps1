...
Run $MSBuild @((Join-Path $Build 'BoozyBardGuard.vcxproj'), '/t:ClCompile', '/p:Configuration=Release', '/p:Platform=x64', '/p:BuildProjectReferences=false', '/verbosity:minimal')
...
$Dll = Join-Path $Build 'Release/BoozyBardGuard.dll'
...
$Info = [ordered]@{
 plugin = 'BoozyBardGuard'; version = '1.3.0'; runtime = '1.6.1170'
 ...
 windows_build = 'passed'; policy_tests = 'passed'; in_game_tested = $false
}
$Info | ConvertTo-Json | Set-Content (Join-Path $Stage 'BuildInfo.json') -Encoding utf8
Compress-Archive -Path (Join-Path $Stage '*') -DestinationPath (Join-Path $WorkDirectory 'Boozy_Bard_Guard_v1_3_native.zip')
