...
Run $MSBuild @((Join-Path $Build 'PhialLifetimeTrace.vcxproj'), '/t:ClCompile', '/p:Configuration=Release', '/p:Platform=x64', '/p:BuildProjectReferences=false', '/verbosity:minimal')
...
$Dll = Join-Path $Build 'Release/PhialLifetimeTrace.dll'
...
foreach ($Dependency in @('minhook','spdlog','fmt','rapidcsv')) {
...
$SourceStage = Join-Path $Stage 'Source/PhialLifetimeTrace'
New-Item -ItemType Directory -Force -Path $SourceStage | Out-Null
foreach ($Name in @('src','tools','CMakeLists.txt','vcpkg.json','README.md','LICENSE')) {
    Copy-Item (Join-Path $Root $Name) $SourceStage -Recurse
}
Copy-Item (Join-Path $Root '../.github/workflows/windows-phial-lifetime-trace.yml') $SourceStage
$Sources = [ordered]@{}
foreach ($Name in @('src/main.cpp','CMakeLists.txt','vcpkg.json','tools/build_windows.ps1','README.md')) {
...
$Info = [ordered]@{
    plugin = 'PhialLifetimeTrace'; version = '0.1.0'; purpose = 'root-cause diagnostic, not a fix'; runtime = '1.6.1170'
    ...
    source_sha256_lf = $Sources; windows_build = 'passed'; in_game_tested = $false
}
$Info | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $Stage 'BuildInfo.json') -Encoding utf8
Compress-Archive -Path (Join-Path $Stage '*') -DestinationPath (Join-Path $WorkDirectory 'Phial_Lifetime_Trace_v0_1.zip')
