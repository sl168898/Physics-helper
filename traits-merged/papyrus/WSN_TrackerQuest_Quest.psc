...
function ProcessFavorChanges(Bool abShowStatusMessages)
	...
	if WorshipID != -1
        ...
        If FDActive() && NewFavor > 400.0
            FavorChangeQueued -= NewFavor - 400.0
            NewFavor = 400.0
        EndIf
		self.SetFavor(NewFavor)
        If FDActive()
            NewFavor = WSN_Favor_Global.GetValue()
        EndIf
...
function SetFavor(Float akFavor)
	...
	if WorshipID >= 0 && WSN_DivineTypeID[WorshipID] == 2 && PlayerRef.HasMagicEffect(WSN_Item_ElvenLamp_Effect_Ench)
		akFavor *= 1.10000
	endIf
    If FDActive() && akFavor > 400.0
        akFavor = 400.0
    EndIf
...
Bool Function FDActive()
    Spell trait = Game.GetFormFromFile(0x900, "Biggie Traits - Combined.esp") as Spell
    GlobalVariable activated = Game.GetFormFromFile(0x920, "Biggie Traits - Combined.esp") as GlobalVariable
    Return trait && activated && activated.GetValue() == 1.0 && PlayerRef.HasSpell(trait)
EndFunction

Function FDRemoveTrait(Float previousTarget)
    WSN_FavoredDiminishTarget = previousTarget
    If WorshipID >= 0 && WSN_Favor_Global.GetValue() > previousTarget
        Float excess = WSN_Favor_Global.GetValue() - previousTarget
        FavorChangeQueued -= excess
        ; Avoid multiplying an already-final value by the lantern a second time.
        Float value = previousTarget
        If WSN_DivineTypeID[WorshipID] == 2 && PlayerRef.HasMagicEffect(WSN_Item_ElvenLamp_Effect_Ench)
            value = previousTarget / 1.1
        EndIf
        SetFavor(value)
    EndIf
EndFunction
