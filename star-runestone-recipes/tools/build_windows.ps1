...
Run $MSBuild @((Join-Path $Build 'StarRunestoneRecipes.vcxproj'), '/t:ClCompile', '/p:Configuration=Release', '/p:Platform=x64', '/p:BuildProjectReferences=false', '/verbosity:minimal')
# Run independent rules and ABI checks before the full support-library build.
Run 'cmake' @('--build', $Build, '--config', 'Release', '--target', 'crafting_tests', 'abi_tests', '--parallel', '2')
...
$Dll = Join-Path $Build 'Release/StarRunestoneRecipes.dll'
if (!(Test-Path $Dll) -or (Get-Item $Dll).Length -eq 0) { throw 'No DLL produced' }
Copy-Item $Dll $PluginDirectory
Run 'python' @((Join-Path $Root 'tools/build_plugin.py'), (Join-Path $Stage 'Star Runestone Recipes.esp'))
Copy-Item (Join-Path $Root 'README.md') $Stage
Copy-Item (Join-Path $Root 'LICENSE') $Stage
Copy-Item (Join-Path $Common 'LICENSE') (Join-Path $Stage 'CommonLibSSE-LICENSE')
$Sources = [ordered]@{}
foreach ($Name in @('src/main.cpp','src/CraftingRules.h','tests/crafting_tests.cpp','tests/abi_tests.cpp','tests/test_plugin.py','tools/build_plugin.py','tools/build_windows.ps1','CMakeLists.txt','vcpkg.json','README.md')) {
...
$Info = [ordered]@{
    plugin = 'StarRunestoneRecipes'; version = '1.0.0'; runtime = '1.6.1170'
    ...
    runemaster_version = '1.5'; recipes = 4; added_powers = 0; star_preserved = $true; crafting_transaction_tests = 'passed'; inventory_hook_abi_tests = 'passed'; binary_plugin_tests = 'passed'
}
$Info | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $Stage 'BuildInfo.json') -Encoding utf8
Compress-Archive -Path (Join-Path $Stage '*') -DestinationPath (Join-Path $WorkDirectory 'Star_Runestone_Recipes_v1_0.zip')
