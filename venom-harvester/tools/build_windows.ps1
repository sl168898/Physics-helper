...
$Sources = [ordered]@{}
foreach ($Name in @('src/main.cpp','src/Harvest.h','src/MenuGate.h','src/CraftCapture.h','tests/harvest_tests.cpp','tests/menu_tests.cpp','tests/crafting_tests.cpp','CMakeLists.txt','vcpkg.json')) {
...
$Info = [ordered]@{
    plugin = 'VenomHarvester'; version = '2.0.3'; runtime = '1.6.1170'
...
Compress-Archive -Path (Join-Path $Stage '*') -DestinationPath (Join-Path $WorkDirectory 'Huntsmans_Satchel_SKSE_v2_0_3.zip')
