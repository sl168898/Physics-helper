...
$Sources = [ordered]@{}
foreach ($Name in @('src/main.cpp','src/Harvest.h','src/MenuGate.h','src/CraftCapture.h','src/DeferredForms.h','tests/harvest_tests.cpp','tests/menu_tests.cpp','tests/crafting_tests.cpp','tests/inventory_event_tests.cpp','CMakeLists.txt','vcpkg.json')) {
...
$Info = [ordered]@{
    plugin = 'VenomHarvester'; version = '2.0.6'; runtime = '1.6.1170'
    ...
    source_sha256_lf = $Sources; windows_build = 'passed'; harvest_tests = 'passed'; menu_tests = 'passed'; crafting_tests = 'passed'; inventory_event_tests = 'passed'; in_game_tested = $false
...
Compress-Archive -Path (Join-Path $Stage '*') -DestinationPath (Join-Path $WorkDirectory 'Huntsmans_Satchel_SKSE_v2_0_6_beta1.zip')
