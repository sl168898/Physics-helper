...
namespace
{
    ...
    struct InventoryFormPolicy
    {
        ...
        static void release(std::uint32_t id)
        {
            ...
            SKSE::log::info("Inventory-event tasks passed; releasing temporary reference to {:08X}", id);
    ...
    struct Craft
    {
        ...
        void finish()
        {
            ...
            if (!tasks) {
                SKSE::log::error("Batch {}: task queue unavailable; inventory left intact", nonce);
                return;
            }
            const std::array<std::uint32_t, 2> eventForms{output.item, poisonID};
            auto inventoryLease = InventoryForms::retain(generation, eventForms);
            if (!inventoryLease) {
                SKSE::log::warn("Batch {}: could not retain inventory-event forms; inventory left intact", nonce);
                return;
            }
            {
                std::lock_guard lock(mutex);
            ...
            // PAPER 2.2.4 resolves batched container-event FormIDs later on the
            // SKSE task queue. Removing the last original bottle used to allow
            // its dynamic form to die before PAPER's queued filter read it.
            // Hold both forms BEFORE removal, then queue release AFTER every
            // event generated by the inventory swap. SKSE processes tasks FIFO.
...
extern "C" __declspec(dllexport) constinit SKSE::PluginVersionData SKSEPlugin_Version = [] {
    ...
    data.PluginVersion({2, 0, 6, 0}); data.PluginName("VenomHarvester");
...
extern "C" __declspec(dllexport) bool SKSEPlugin_Load(const SKSE::LoadInterface* skse)
{
    ...
    *path /= "VenomHarvester.log";
    ...
    SKSE::Init(skse);
    SKSE::log::info("Huntsman's Satchel 2.0.6 beta; Skyrim 1.6.1170; native AddPoison; inventory forms retained through queued events");
