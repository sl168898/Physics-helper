...
Function Refill(Potion fillItem)
    ; Guard before alias clearing, inventory removal or world reference creation.
    If WPD_Blacklist.IsBlocked(fillItem) || !WPD_Storage.IsReady()
        Return
    EndIf
...
;		Debug.Trace("MS12: White Phial in a container: " + CurrentContainer)
		(GetOwningQuest() As MS12PostQuestScript).RemoveDuplicateWhitePhial(CurrentContainer)
...
Function SetForRefill(Actor drinker)
	(GetOwningQuest() As MS12PostQuestScript).RemoveDuplicateWhitePhial(drinker)
...
;	GetOwningQuest().RegisterForSingleUpdateGameTime(RefillTime)
	GetOwningQuest().RegisterForSingleUpdateGameTime(TWPTE_ResetHours.GetValue()) ; edit
EndFunction
; /71753
