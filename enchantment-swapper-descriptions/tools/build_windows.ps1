...
Run $MSBuild @((Join-Path $Build 'EnchantmentSwapperDescriptions.vcxproj'), '/t:ClCompile', '/p:Configuration=Release', '/p:Platform=x64', '/p:BuildProjectReferences=false', '/verbosity:minimal')
...
$Dll = Join-Path $Build 'Release/EnchantmentSwapperDescriptions.dll'
...
Compress-Archive -Path (Join-Path $Stage '*') -DestinationPath (Join-Path $WorkDirectory 'Enchantment_Swapper_Descriptions_Native_v1.zip')
