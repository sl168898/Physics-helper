...
Run $MSBuild @((Join-Path $Build 'BatchCrafting.vcxproj'), '/t:ClCompile', '/p:Configuration=Release', '/p:Platform=x64', '/p:BuildProjectReferences=false', '/verbosity:minimal')
...
$Dll = Join-Path $Build 'Release/BatchCrafting.dll'
...
$SourceDirectory = Join-Path $Stage 'Source/BatchCrafting'
New-Item -ItemType Directory -Force -Path $SourceDirectory | Out-Null
foreach ($Name in @('src','tests','tools','CMakeLists.txt','vcpkg.json','LICENSE','README.md','BatchCrafting.ini')) {
